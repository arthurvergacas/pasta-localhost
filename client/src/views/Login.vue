	...
	<div class="login">
		...
		<form @submit.prevent="login">
			<input
				...
				id="input"
			/>
			<button type="submit" @keypress.enter="login" id="button">Entrar</button>
		</form>

		<div class="container message-wrapper d-flex flex-column">
			<div class>
				<span id="user-not-found" v-bind:class="{ logged: auth }">
					{{ message }}
				</span>
			</div>

			<div class>
				<button id="button" @click="goToSignIn">Cadastre-se</button>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		login() {
			...
			if (name === "") {
				this.message = "Digite seu nome de usuário";
				return;
			}
			this.isLoading = true;
			...
			fetch(`${API_URL}/log-in`, {
				...
				.then((result) => {
					// set auth to true
					this.auth = true;
					this.message = "Logado com sucesso! Redirecionando...";
